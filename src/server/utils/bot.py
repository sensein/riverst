...
from pipecat.frames.frames import EndFrame, UserStartedSpeakingFrame
...
async def run_bot(webrtc_connection: Any, config: dict, session_dir: str) -> None:
    ...
    task = PipelineTask(
        ...
        idle_timeout_frames=UserStartedSpeakingFrame,
    )
    ...
    async def on_audio_data(_, audio, sr, ch):
        """Handle audio data from the audio buffer and save it to a file."""
