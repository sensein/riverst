...
class ProfileAnalyzer:
    ...
    def print_latency_analysis(self):
        ...
        for metric_key, metric_name in key_metrics:
            if metric_key in analysis:
                ...
                if stats['count'] > 0:
                    print(f"{metric_name:20} | Avg: {stats['avg']:.3f}s | "
    ...
    def print_bottleneck_analysis(self):
        ...
        if 'slowest_stage' in bottlenecks:
            ...
            print(f"Slowest Stage: {slowest['name']} ({slowest['avg_duration']:.3f}s avg)")
            print()
    ...
    def print_frame_statistics(self):
        ...
        sorted_frames = sorted(frame_stats.items(), key=lambda x: x[1]['count'], reverse=True)
        ...
        for frame_type, stats in sorted_frames:
            ...
            print(f"{frame_type:25} | {count:8,} | {avg_size:8,.0f}B | {total_mb:8.2f}MB")
        print()
    ...
    def generate_performance_report(self) -> str:
        ...
        if 'total_turn_latency' in analysis:
            ...
            if total_stats['count'] > 0:
                ...
                report_lines.append(f"  Average Turn Latency: {total_stats['avg']:.3f}s")
        ...
        if 'slowest_stage' in bottlenecks:
            ...
            report_lines.append(f"  Primary bottleneck: {slowest['name']} ({slowest['avg_duration']:.3f}s)")
        ...
        if bottlenecks and 'slowest_stage' in bottlenecks:
            ...
            if 'llm' in slowest_name:
                ...
                report_lines.append("  - Enable streaming for faster perceived response")
    ...
    def compare_sessions(self, other_session_dir: str):
        ...
        print(f"{'Metric':20} | {'Session A':>10} | {'Session B':>10} | {'Difference':>12}")
        print("-" * 65)
        ...
        for metric_key, metric_name in metrics_to_compare:
            if metric_key in analysis1 and metric_key in analysis2:
                ...
                print(f"{metric_name:20} | {avg1:8.3f}s | {avg2:8.3f}s | {diff:+7.3f}s ({diff_pct:+.1f}%)")
