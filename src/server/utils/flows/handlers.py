...
async def general_transition_callback(
    ...
    if result.get("status") == "success":
        ...
        message = result.get(
            "message", "Please ensure you have completed all required checklist items."
        )
        message = (
            "CRITICAL INSTRUCTION: "
            + "You are rejoining the flow at a point where you have not completed all required tasks."
            + " LOOK AT YOUR INSTRUCTIONS CAREFULLY AND MAKE SURE YOU UNDERSTAND THE TASKS YOU NEED TO COMPLETE, "
            + " and SKIP ANY TASKS THAT I HAVE NOT ASKED YOU TO COMPLETE AGAIN: "
            + message
        )
        await flow_manager.set_node(
            flow_manager.current_node, create_current_node(flow_manager, message)
        )
...
async def get_session_variable_handler(
    ...
    index_error_message = (
        ...
        "(valid indices: {options}). Please ask the user for a proper index in very "
        "natural language given the context, then call this function again with "
        "'current_index' set to their response. If there was an error in how they "
        "responded, please try to correct the user in a very natural way."
    )
