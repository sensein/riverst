...
def generate_json():
    try:
        ...
        # Format the received data into the desired JSON structure
        flow_data = {
            "name": data.get("name", ""),
        ...
        for i, node in enumerate(nodes):
            ...
            if i < len(nodes) - 1:
                node_data["next_stage"] = nodes[i + 1]["node_name"]
            else:
                node_data["next_stage"] = "end"
            ...
            flow_data["state_config"]["stages"][node_name] = {
                "checklist": checklist,
            ...
            flow_data["schemas"][f"{node_name}_schema"] = schema
