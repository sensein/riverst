...
class Function(BaseModel):
    ...
    def validate_function(cls, function):
        ...
        if not function.transition_callback and not function.handler:
            raise ValueError(
                "Function must include either transition_callback or handler"
            )
...
class Node(BaseModel):
    ...
    def validate_functions(cls, functions, info):
        ...
        if info.data.get("name") != "end" and len(functions) < 1:
            raise ValueError(
                f"Node functions list must have at least one element, found {len(functions)}"
            )
...
class NodesConfig(BaseModel):
    ...
    def validate_node_config(self):
        ...
        if not isinstance(initial_node, dict):
            initial_node = (
                initial_node.model_dump()
                if hasattr(initial_node, "model_dump")
                else initial_node
            )
        ...
        for node_id, node in self.nodes.items():
            ...
            if "functions" not in node_dict or not isinstance(
                node_dict["functions"], list
            ):
