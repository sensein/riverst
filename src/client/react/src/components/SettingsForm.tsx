...
const SettingsForm: React.FC<SettingsFormProps> = ({ schema, onSubmit }) => {
  ...
  const [dynamicEnums, setDynamicEnums] = useState<{
    books: { id: string; path: string; title: string }[];
  }>({ books: [] });
  ...
  const defaultAnimations =
    ...
    [];
  const defaultCamera =
    (schema.properties?.options as any)?.properties?.camera_settings?.default ??
    "upper";

  const isObjectSchema = (
    def: JSONSchema7Definition | undefined,
  ): def is JSONSchema7 => typeof def === "object" && def !== null;

  const options =
    isObjectSchema(schema?.properties?.options) &&
    "properties" in schema.properties.options
      ? schema.properties.options.properties || {}
      : {};

  const requiredFields =
    isObjectSchema(schema?.properties?.options) &&
    "required" in schema.properties.options
      ? schema.properties.options.required || []
      : [];

  const schemaName =
    isObjectSchema(schema?.properties?.name) &&
    "const" in schema.properties.name
      ? schema.properties.name.const
      : "";

  const schemaDescription =
    isObjectSchema(schema?.properties?.description) &&
    "const" in schema.properties.description
      ? schema.properties.description.const
      : "";

  const pipelineModality = Form.useWatch(
    ["options", "pipeline_modality"],
    form,
  );
  const embodiment = Form.useWatch(["options", "embodiment"], form);
  const selectedBook = Form.useWatch(
    ["options", "activity_variables_path"],
    form,
  );
  ...
  useEffect(() => {
    ...
    const optionsSchema =
      (schema.properties?.options as JSONSchema7)?.properties || {};
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({
      options: {
        ...currentOptions,
        body_animations:
          embodiment === "humanoid_avatar" ? defaultAnimations : [],
        camera_settings:
          embodiment === "humanoid_avatar" ? defaultCamera : undefined,
      },
    });
  }, [embodiment, form]);
  ...
  const renderFormItem = (key: string, config: any) => {
    ...
    if (
      ["stt_type", "tts_type"].includes(key) &&
      pipelineModality !== "classic"
    )
      return null;

    if (key === "llm_model" && pipelineModality !== "e2e") return null;
    ...
    // Free-text string array (tag input) — e.g. vocab_override
    if (config.type === "array" && config.items?.type === "string" && !config.items?.enum) {
      return (
        <Form.Item
          key={key}
          name={namePath}
        ...
        rules.push({
          validator: (_: any, value: any) =>
            Array.isArray(value) && value.length < config.minItems
              ? Promise.reject(
                  new Error(
                    `Please select at least ${config.minItems} item(s).`,
                  ),
                )
              : Promise.resolve(),
        });
    ...
    if (
      config.type === "integer" ||
      (Array.isArray(config.type) && config.type.includes("integer"))
    ) {
      const max = key === "index" && maxIndices ? maxIndices : config.maximum;

      // Add validation rules for resource index
      if (key === "index" && maxIndices) {
        const indexTypeSingular =
          indexType.slice(-1) === "s" ? indexType.slice(0, -1) : indexType;
        rules.push({
          validator: (_: any, value: any) => {
            ...
            if (!Number.isInteger(numValue)) {
              return Promise.reject(
                new Error(`${indexTypeSingular} must be a whole number`),
              );
            }
            if (numValue < 1) {
              return Promise.reject(
                new Error(`${indexTypeSingular} must be at least 1`),
              );
            }
            if (numValue > maxIndices) {
              return Promise.reject(
                new Error(
                  `${indexTypeSingular} must be no more than ${maxIndices}`,
                ),
              );
            }
            return Promise.resolve();
      ...
      return (
        ...
        >
          <Input
            ...
            max={max}
            placeholder={
              key === "index" && maxIndices ? `1-${maxIndices}` : undefined
            }
          />
        </Form.Item>
      );
  ...
  const defaultOptions = Object.entries(options).reduce<Record<string, any>>(
    (acc, [key, def]) => {
      if (typeof def !== "object" || def === null) return acc;
      acc[key] =
        "default" in def
          ? (def as any).default
          : (def as any).type === "boolean"
            ? false
